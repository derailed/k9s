...
func (l *LogItems) Lines(index int, logOptions *LogOptions, ll [][]byte) {
	l.mx.Lock()
	defer l.mx.Unlock()
	...
	for i, item := range l.items[index:] {
		...
		item.Render(color, logOptions, bb)
		ll[i] = bb.Bytes()
	}
}

// StrLines returns a collection of log lines.
func (l *LogItems) StrLines(index int, logOptions *LogOptions) []string {
	l.mx.Lock()
	defer l.mx.Unlock()
	...
	for i, item := range l.items[index:] {
		bb := bytes.NewBuffer(make([]byte, 0, item.Size()))
		item.Render("white", logOptions, bb)
		ll[i] = bb.String()
	}
...
func (l *LogItems) Render(index int, logOptions *LogOptions, ll [][]byte) {
	var colorIndex int
	for i, item := range l.items[index:] {
		id := item.ID()
		color, ok := l.podColors[id]
...
func (l *LogItems) Filter(index int, q string, logOptions *LogOptions) ([]int, [][]int, error) {
	if q == "" {
		return nil, nil, nil
	}
	if f, ok := internal.IsFuzzySelector(q); ok {
		mm, ii := l.fuzzyFilter(index, f, logOptions)
		return mm, ii, nil
	}
	matches, indices, err := l.filterLogs(index, q, logOptions)
	if err != nil {
		return nil, nil, err
	}
...
func (l *LogItems) fuzzyFilter(index int, q string, logOptions *LogOptions) ([]int, [][]int) {
	q = strings.TrimSpace(q)
	matches, indices := make([]int, 0, len(l.items)), make([][]int, 0, 10)
	mm := fuzzy.Find(q, l.StrLines(index, logOptions))
	for _, m := range mm {
...
func (l *LogItems) filterLogs(index int, q string, logOptions *LogOptions) ([]int, [][]int, error) {
	var invert bool
