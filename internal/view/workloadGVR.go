...
const workloadGVRTitle = "CustomGVR"
...
func (a *WorkloadGVR) bindKeys(aa *ui.KeyActions) {
	...
	aa.Bulk(ui.KeyMap{
		...
		ui.KeyR: ui.NewKeyActionWithOpts("Remove custom GVR", a.removeCustomCmd, ui.ActionOpts{
			Visible:   true,
			Dangerous: true,
		}),
		...
		ui.KeyShiftA: ui.NewKeyActionWithOpts("Add To Current Context", a.addtoCurrentCtx,
			ui.ActionOpts{
				Visible:   true,
				Dangerous: true,
			}),
		ui.KeyShiftD: ui.NewKeyActionWithOpts("Delete From Current Context", a.deletefromCurrentCtx,
			ui.ActionOpts{
				Visible:   true,
				Dangerous: true,
			}),
