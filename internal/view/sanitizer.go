...
func (s *Sanitizer) ExtraHints() map[string]string {
	...
	return xray.EmojiInfo(s.app.Config.K9s.UI.Emoji)
}
...
func (s *Sanitizer) update(node *xray.TreeNode) {
	root := makeTreeNode(node, s.ExpandNodes(), s.app.Config.K9s.UI, s.app.Styles)
	if node == nil {
		s.app.QueueUpdateDraw(func() {
			s.SetRoot(root)
		})
...
func (s *Sanitizer) hydrate(parent *tview.TreeNode, n *xray.TreeNode) {
	node := makeTreeNode(n, s.ExpandNodes(), s.app.Config.K9s.UI, s.app.Styles)
