...
func (t *Table) isVisible(h model1.HeaderColumn, isNamespaced *bool) bool {
	if h.Name == "NAMESPACE" && (!t.GetModel().ClusterWide() || (isNamespaced != nil && !*isNamespaced)) {
		return false
	}
...
func (t *Table) buildRow(r int, re, ore model1.RowEvent, h model1.Header, pads MaxyPad, isNamespaced bool) {
	...
	for c, field := range re.Row.Fields {
		...
		if !t.isVisible(h[c], &isNamespaced) {
			continue
		}
