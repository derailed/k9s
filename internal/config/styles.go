...
type (
	...
	StatusEmoji struct {
		...
		CheckMark string `json:"check_mark" yaml:"check_mark"`
	}
...
func (e *EmojiConfig) EmojiFor(key string) string {
	...
	case "files":
		switch subKey {
		case "file":
			return e.K9s.Files.File
		case "folder":
			return e.K9s.Files.Folder
		}
	case "prompt":
		switch subKey {
		case "query":
