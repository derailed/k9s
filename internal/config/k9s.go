...
type K9s struct {
	...
	CustomWorkloadGVRs  []WorkloadGVR `json:"customWorkloadGVRs,omitempty" yaml:"customWorkloadGVRs,omitempty"`
...
func NewK9s(conn client.Connection, ks data.KubeSettings) *K9s {
	return &K9s{
		...
		CustomWorkloadGVRs: NewDefaultWorkloadGVRs(),
...
func (k *K9s) Merge(k1 *K9s) {
	...
	// TODO: comment, we don't want to update the list if not set, this should use the default values instead
	if k1.CustomWorkloadGVRs != nil {
		k.CustomWorkloadGVRs = k1.CustomWorkloadGVRs
	}
}
