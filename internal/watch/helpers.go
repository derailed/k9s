...
func searchFQN(res, fqn string) string {
	return res + ":" + fqn
}

func indexFQN(res string, m metav1.ObjectMeta) string {
	return res + ":" + MetaFQN(m)
}
...
// WithPerc shows a combined number and ratio.
func withPerc(v, p string) string {
	var b strings.Builder
	b.Grow(len(v) + len(p) + 2)
	b.WriteString(v)
	b.WriteString("(")
	b.WriteString(p)
	b.WriteString(")")
